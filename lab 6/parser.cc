    ...
    while (true)
    {
        ...
        else
        {
            if (parse_table[top.value][itr->value].arr.size() > 0)
            {
                ...
                {
                    auto act_it = parse_table[top.value][itr->value].action.rbegin();
                    for (auto it = parse_table[top.value][itr->value].arr.rbegin(); it != parse_table[top.value][itr->value].arr.rend(); it++, act_it++) {
                        if (act_it->compare("B_.in=1") == 0) {
                            // cout << "true" << endl;
                            (it-1)->in = 1;
                        } else if(act_it->compare("B_.in=2*B_.val") == 0) {
                            // cout << "true" << endl;
                            (it-1)->in = 2*top.in;
                        } else if(act_it->compare("B_.in=2*B_.val+1") == 0) {
                            (it-1)->in = 2*top.in+1;
                        }
                    }
